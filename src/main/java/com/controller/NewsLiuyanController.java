    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(newsLiuyan !=null){
            ...
            if(news != null){
            BeanUtils.copyProperties( news , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
...
//        else if("用户".equals(role))
//            newsLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(newsLiuyan.getNewsLiuyanText()) || "null".equals(newsLiuyan.getNewsLiuyanText())){
                newsLiuyan.setNewsLiuyanText(null);
        }
        if("".equals(newsLiuyan.getReplyText()) || "null".equals(newsLiuyan.getReplyText())){
                newsLiuyan.setReplyText(null);
        }
        ...
        NewsLiuyanEntity newsLiuyan = newsLiuyanService.selectById(id);
            if(newsLiuyan !=null){
                ...
                //级联表
                    NewsEntity news = newsService.selectById(newsLiuyan.getNewsId());
                if(news != null){
                    BeanUtils.copyProperties( news , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setNewsId(news.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(newsLiuyan.getYonghuId());
